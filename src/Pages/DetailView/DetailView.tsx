...
const DetailView = () => {
  ...
  const { video, loading } = useAppSelector((state) => state.videos);
  ...
  if (loading) {
    return (
      <div className={`${styles.loader} mainContent`}>
        {" "}
        <ReactLoading
          type={"balls"}
          color={"#fff"}
  ...
  return (
    <div className={`mainContent ${styles.mainContent}`}>
      ...
      <section className={`${styles.userSection}`}>
        ...
        <button className={`${styles.subscribe__button} ms-3`} onClick={handleSubscribe}>
          Подписатся
        </button>
